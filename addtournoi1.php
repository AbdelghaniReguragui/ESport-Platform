...
if(isset($_POST['submit']) )
{
    ...
    if(empty($_POST['tourName']) || empty($_POST['size']) || empty($_POST['radio1']) || empty($_POST['select'])  || 
            empty($_POST['radio']) )
            {
                ...
                $requete = mysqli_query($conn,"INSERT INTO `tournoi` (`ID_TOURNOI`, `ID_TOURNOI_CAT`, `ID_USER`,`id_user2`, `NOM_TOURNOI`, `DATE_CREATION`, `DATE`, `SIZE`, `ID_TYPE`, `ID_JEU`,`ID_PLATFORME`) VALUES ('', '$radio1', '1','$id_user2', '$tour', current_timestamp(),  '$date', '$size', '$radio', '$jeu','$select')");
