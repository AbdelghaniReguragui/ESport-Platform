...
<div class="tour-all-games-card">
  <div class="see">
    <span class="title">Tournaments by game</span>
   
    <button class="orgnize"><a href="addtournoi1.php"><i class="fas fa-trophy" style="margin-right:2%;"></i>  Orgnize </a></button>
  </div>
  <?php
    $query = "SELECT DISTINCT jeu.ID_JEU,jeu.IMAGE1,jeu.LOGO,jeu.NOM_JEU FROM jeu INNER JOIN tournoi ON tournoi.ID_JEU = jeu.ID_JEU    ";
    $res = mysqli_query($conn, $query);
    if(mysqli_num_rows($res)>0){
        while($row = mysqli_fetch_assoc($res)){
  ?>
  <div class="tour">
    <a href="affichagetournois1.php?id_jeu=<?php echo($row['ID_JEU']);?>">
        ...
        <div class="tour-game-container">
            <div class="tour-games">
                ...
                <a href ="#"><p class="tour-game-title"><?php echo($row['NOM_JEU']); ?></p></a>
						...
						<a style="width:400px; height:40px; background:white; border-radius:20px; color:black;" href="partage_tournoi.php?id_jeu=<?php echo($row['ID_JEU']);?>"><i class="fas fa-share"></i>&nbsp;&nbsp;Partager</a>
