...
<div class="search-game">
                <table class="elementcontainer">
                    <tr>
                        <td>
                        <button type="submit" style="border:none; background:none;"> <i class="fa" style="color: rgba(55, 98, 166, 1);">&#xf002;</i></button>                     
                        </td>
                        <td>
                        <input  type="text" class="search-game-input" name="q" placeholder="Search Games...">
                        </td>
                    </tr>
                </table>            
  </div>
...
<form method="post">
<div class="cat">
      <button class="cat-sub" type="submit" name="submit"></button>
        <select class="categories-game" name="categories" id="categories">
          <option value="all">All Categories</option> 
          <?php 

            $query1 = "SELECT * FROM jeu_cat";
            $res = mysqli_query($conn, $query1);
            if(mysqli_num_rows($res)>0){
              
              while($row = mysqli_fetch_assoc($res)){
                ?>
          <option name="submit" value="<?php echo($row['ID_JEU_CAT']); ?>"><?php echo($row['NOM_JEU_CAT']); ?></option>
          <?php }
          }
        ?>
        </select>
...
if(isset($_POST['submit'])){
  $selected_val = $_POST['categories'];
  if ($selected_val == "all"){
    $query = "SELECT * FROM jeu ";
  }else{
    $query = "SELECT * FROM jeu WHERE ID_JEU_CAT=$selected_val";
  }
  $res = mysqli_query($conn, $query);
    ...
    if(mysqli_num_rows($res)>0){
        ...
        <div class="game-card">
          <img style="width:100%" src=<?php echo( "data:image/jpeg;base64,".base64_encode($row["IMAGE"])); ?> >
          <div class="game-container">
                <div class="games">
                  <a href ="#"><p class="games-title"><?php echo($row['NOM_JEU']); ?></p></a>
                </div>
                <a href ="#"><p class="game-viewer"><?php echo($row['VIEWERS']); ?> viewers</p></a>
                <div class="subscribe">
                  <button type="submit"> <i class="fas fa-plus-square"></i> Subscribe</button>
                </div>
          </div>
        </div>
  </div>
...
<?php
    ...
    if(empty($_GET['q']) && !isset($_POST['submit'])) {
    $query = "SELECT * FROM jeu ";
  ...
  if(isset($_GET['q']) AND !empty($_GET['q']) AND !isset($_POST['submit'])) {
